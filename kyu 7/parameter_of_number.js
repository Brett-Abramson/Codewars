...
const parameter = (n) => {
  ...
  const sumProduct = arrayOfDigits.reduce(
    (total, digit) => {
      const num = Number(digit);
      return (total = [total[0] + num, num * total[1]]);
    },
    [0, 1]
  );

  // console.log(sumProduct)
  let sum = sumProduct[0];
  let product = sumProduct[1];

  if (sum === product) return sum;
  else {
    for (let lcm = sum; lcm <= product * sum; lcm += sum) {
      if (lcm % sum === 0 && lcm % product === 0) {
        return lcm;
      }
    }
  }
